...
export default function HomeClient() {
  ...
  if (!todayPrices.length) {
    return (
      <div style={{ textAlign: 'center', padding: '100px 20px', color: 'var(--text-soft)' }}>
        Error cargando datos. Vuelve a intentarlo en unos minutos.
      </div>
    );
  }
...
function HomeContent({
  ...
  return (
    <>
      ...
      {/* H1 SEO - oculto visualmente pero presente para Google */}
      <h1 className="sr-only">Precio de la luz hoy por horas en España</h1>
      ...
      <div className="hero-action">
        <div className="hero-grid">
          <div className="hero-block">
            ...
            <div className={`urgent-msg ${urgentCls}`}>
              <UrgentIcon size={16} style={{ flexShrink: 0 }} />
              <span>{urgentMsg}</span>
            </div>
          </div>
          ...
          <div className="hero-block divider">
            <div className="hero-label">
              <Moon size={14} className="icon-muted" style={{ verticalAlign: 'middle', marginRight: '4px' }} />
              Mejor hora {isToday ? 'hoy' : 'mañana'}
            </div>
          ...
          <div className="hero-block divider">
            <div className="hero-label">
              <BarChart3 size={14} className="icon-muted" style={{ verticalAlign: 'middle', marginRight: '4px' }} />
              Resumen del día
            </div>
        ...
        <div className="semaphore">
          <div className={`sem green ${sc === 'green' ? 'active' : ''}`}>
            <div className="sem-row">
              <Circle size={20} fill="var(--green-bright)" strokeWidth={0} />
              <div className="sem-title">Barato</div>
            </div>
            <div className="sem-desc">Menos de 10 c€/kWh — enciende lo que quieras</div>
          </div>
          <div className={`sem yellow ${sc === 'yellow' ? 'active' : ''}`}>
            <div className="sem-row">
              <Circle size={20} fill="var(--yellow)" strokeWidth={0} />
              <div className="sem-title">Moderado</div>
            </div>
            <div className="sem-desc">Entre 10 y 17 c€/kWh — con moderación</div>
          </div>
          <div className={`sem red ${sc === 'red' ? 'active' : ''}`}>
            <div className="sem-row">
              <Circle size={20} fill="var(--red)" strokeWidth={0} />
              <div className="sem-title">Caro</div>
            </div>
      ...
      {/* CALCULADORAS - 4 destacadas como recomienda el plan SEO */}
      <section className="calc-shortcuts">
        <h2 className="calc-shortcuts-title">Calcula cuánto te cuesta usar tus aparatos hoy</h2>
        <div className="calc-shortcuts-grid">
          <Link href="/calculadoras/lavadora" className="calc-shortcut">
            <Shirt size={24} className="icon-accent" />
            <div>
              <div className="calc-shortcut-title">Lavadora</div>
              <div className="calc-shortcut-desc">Cuánto cuesta cada lavado</div>
            </div>
          </Link>
          <Link href="/calculadoras/coche-electrico" className="calc-shortcut">
            <Car size={24} className="icon-accent" />
            <div>
              <div className="calc-shortcut-title">Coche eléctrico</div>
              <div className="calc-shortcut-desc">Cuánto cuesta cargarlo</div>
            </div>
          </Link>
          <Link href="/calculadoras/aire-acondicionado" className="calc-shortcut">
            <Snowflake size={24} className="icon-accent" />
            <div>
              <div className="calc-shortcut-title">Aire acondicionado</div>
              <div className="calc-shortcut-desc">Consumo por hora</div>
            </div>
          </Link>
          <Link href="/calculadoras/consumo-electrico" className="calc-shortcut">
            <ClipboardList size={24} className="icon-accent" />
            <div>
              <div className="calc-shortcut-title">Consumo general</div>
              <div className="calc-shortcut-desc">De vatios a euros</div>
            </div>
          </Link>
        </div>
      </section>
          ...
          <table className="price-table">
            ...
            <tbody>
              {prices.map(p => {
                ...
                const sClass = semClass(p.price);
                const sText = p.price < 0.08 ? 'Muy barato' : p.price < 0.10 ? 'Barato' : p.price < 0.14 ? 'Moderado' : p.price < 0.18 ? 'Alto' : 'Caro';
                return (
                  <tr key={p.hour} className={`${isCur ? 'now-row' : ''}${isBest ? ' best-row' : ''}`}>
                    ...
                    <td><span className={`pill ${sClass}`}>{sText}</span></td>
                  </tr>
                );
              })}
            </tbody>
      ...
      {/* TEXTO SEO - contexto útil entre 250-450 palabras */}
      <section className="seo-context">
        <h2>Cómo funciona el precio de la luz en España</h2>
        <p>
          El precio de la luz en España cambia cada hora y se publica diariamente para el día siguiente. Esto significa que el coste de poner una lavadora, encender el aire acondicionado o cargar un coche eléctrico depende mucho de la hora en que lo hagas. En CuantoLuz mostramos el precio PVPC (Precio Voluntario para el Pequeño Consumidor) actualizado con los datos oficiales de Red Eléctrica de España.
        </p>
        <p>
          El PVPC es la tarifa regulada por el Gobierno y la usan más de 8 millones de hogares en España. Si no sabes qué tarifa tienes, probablemente sea esta. Su precio cambia hora a hora según la demanda y la generación, por eso conviene consumir cuando es más barato.
        </p>
        <h2>Qué información encontrarás aquí</h2>
        <p>
          Arriba puedes ver el precio actual, la hora más barata y la más cara del día, junto con el resumen completo en la tabla. También tenemos calculadoras para saber cuánto te va a costar usar cada electrodoméstico hoy. Y si quieres entender mejor cómo funciona todo, tenemos guías sobre el <Link href="/guias/que-es-pvpc">PVPC</Link>, la diferencia entre <Link href="/guias/mercado-regulado-vs-mercado-libre">mercado regulado y libre</Link>, y trucos para <Link href="/guias/como-ahorrar-luz-en-casa">ahorrar en tu factura</Link>.
        </p>
      </section>

      {/* RELATED LINKS - Interlinking para SEO */}
      <RelatedLinks
        title="Sigue explorando"
        links={[
          {
            title: '¿Qué es el PVPC?',
            description: 'Cómo funciona la tarifa regulada que usan más de 8 millones de hogares.',
            url: '/guias/que-es-pvpc',
          },
          {
            title: 'Precio de la luz mañana',
            description: 'Consulta los precios del día siguiente cuando se publican.',
            url: '/precio-luz-manana',
          },
          {
            title: 'Cuánto cuesta poner una lavadora',
            description: 'Guía y calculadora con el precio real según la hora.',
            url: '/electrodomesticos/cuanto-cuesta-poner-una-lavadora',
          },
          {
            title: 'Cómo ahorrar luz en casa',
            description: 'Trucos prácticos para reducir tu factura sin renunciar a comodidad.',
...
function HomeStyles() {
  return (
    <style jsx global>{`
      ...
      .hero-label { font-size: 11px; text-transform: uppercase; letter-spacing: 2px; color: var(--text-soft); margin-bottom: 10px; display: flex; align-items: center; gap: 4px; }
      ...
      .urgent-msg { margin-top: 16px; padding: 10px 14px; border-radius: 10px; font-size: 13px; font-weight: 500; display: flex; align-items: center; gap: 8px; }
      ...
      .seo-context h2:not(:first-child) {
        margin-top: 24px;
      }
      ...
      @media (max-width: 767px) {
        ...
        .hero-action { padding: 24px 20px; }
        .seo-context { padding: 22px 18px; }
      }
    `}</style>
  );
}
