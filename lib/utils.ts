...
export const appliances: Record<string, Appliance> = {
  lavadora_basica:    { label: 'Lavadora estándar',         icon: Shirt,          kwh: 0.9,  duration: 2.0,  type: 'cycle' },
  lavadora_a:         { label: 'Lavadora eficiente (A)',    icon: Shirt,          kwh: 0.55, duration: 1.5,  type: 'cycle' },
  secadora_basica:    { label: 'Secadora estándar',         icon: Wind,           kwh: 3.5,  duration: 1.0,  type: 'cycle' },
  secadora_a:         { label: 'Secadora eficiente (A)',    icon: Wind,           kwh: 1.6,  duration: 1.0,  type: 'cycle' },
  lavavajillas:       { label: 'Lavavajillas estándar',     icon: UtensilsCrossed, kwh: 1.3,  duration: 2.5,  type: 'cycle' },
  lavavajillas_a:     { label: 'Lavavajillas eficiente',    icon: UtensilsCrossed, kwh: 0.85, duration: 2.0,  type: 'cycle' },
  aire_inverter:      { label: 'Aire acond. inverter',      icon: Snowflake,      kw: 1.0,   type: 'time' },
  aire_antiguo:       { label: 'Aire acond. antiguo',       icon: Snowflake,      kw: 2.5,   type: 'time' },
  calefaccion:        { label: 'Calefacción eléctrica',     icon: Flame,          kw: 2.0,   type: 'time' },
  termo:              { label: 'Termo eléctrico',           icon: Droplets,       kw: 1.5,   type: 'time' },
  tv:                 { label: 'TV LED',                    icon: Tv,             kw: 0.1,   type: 'time' },
  ordenador:          { label: 'Ordenador / PC',            icon: Monitor,        kw: 0.2,   type: 'time' },
  estufa:             { label: 'Estufa eléctrica',          icon: Flame,          kw: 1.5,   type: 'time' },
};

export const cars: Record<string, { label: string; per100: number; battery: number }> = {
